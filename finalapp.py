...
def search_records():
    ...
    c.execute("SELECT id, * FROM crmt WHERE last_name like ?", (lookup_record,))
    records = c.fetchall()
...
'''
for record in data:
	c.execute("INSERT INTO crmt VALUES (?, ?, ?, ?, ?, ?, ?)", 
		{
		'first_name': record[0],
		'last_name': record[1],
		'id': record[2],
...
def remove_one():
    ...
    c.execute("DELETE from crm.dbo.crmt WHERE id=" + id_entry.get())
...
def remove_many():
    ...
    if response == 1:
        ...
        c.executemany("DELETE FROM crmt WHERE id = ?", [(a,) for a in ids_to_delete])
...
def remove_all():
    ...
    if response == 1:
        ...
        # Delete Everything From The Table
        c.execute("DROP TABLE crm.dbo.crmt")
...
def update_record():
    ...
    c.execute("""UPDATE crm.dbo.crmt SET
		first_name = ?,
		last_name = ?,
		address = ?,
		city = ?,
		...
		WHERE id = ?""",
              ...
              city_entry.get(),
              state_entry.get(),
              zipcode_entry.get(), id_entry.get(), )
...
def add_record():
    ...
    # Add New Record
    c.execute(
        "INSERT INTO crm.dbo.crmt VALUES (?, ?, ?, ?, ?, ?, ?)",
        fn_entry.get(),
        ln_entry.get(),
        id_entry.get(),
        address_entry.get(),
...
def create_table_again():
    ...
    # Create Table
    c.execute("""CREATE TABLE if not exists crmt (
		first_name text,
		last_name text,
		id integer,
		address text,
